...
const getCalendarDays = (date: Date): Date[] => {
  ...
  const days: Date[] = [];
  for (let i = firstDay - 1; i >= 0; i--) days.push(new Date(year, month - 1, daysInPrevMonth - i));
...
const isCurrentMonth = (date: Date, currentDate: Date) => date.getMonth() === currentDate.getMonth() && date.getFullYear() === currentDate.getFullYear();
...
const generateDayCellData = (date: Date, events: CalendarEvent[], currentDate: Date, selectedDate?: Date) => ({
  date,
...
export const CalendarView = ({ currentDate, events, selectedDate, onDayClick, onEventClick }: CalendarViewProps) => {
