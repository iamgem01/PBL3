...
export default function SidebarTeamspace({ collapsed }: SidebarTeamspaceProps) {
      ...
      try {
        ...
        if (userData) {
          ...
          const userNotes = notesData.filter(
            (note: any) => note.created_by === user.id
          );

          if (userNotes.length !== notesData.length) {
            console.warn(
              `🚨 Filtered ${
                notesData.length - userNotes.length
              } notes that don't belong to current user`
            );
          }

          setNotes(userNotes);
          console.log(
            `✅ Loaded ${userNotes.length} personal notes for user ${user.id}`
          );
        } else {
  ...
  const getNoteIcon = (title: string, content: string) => {
    ...
    if (lowerTitle.includes("project") || lowerContent.includes("brainstorm"))
      return "🧠";
    if (lowerTitle.includes("travel") || lowerContent.includes("trip"))
      return "🧳";
    return "📄";
  };
          ...
          {sharedLoading ? (
            ...
            sharedDocuments.map((item) => (
              ...
              >
                ...
                {!collapsed &&
                  item.note.shares &&
                  item.note.shares.length > 0 && (
                    <span className="text-xs text-blue-500 opacity-0 group-hover:opacity-100 transition-opacity">
                      {item.note.shares.length === 1 &&
                      item.note.shares[0] === "all"
                        ? "All"
