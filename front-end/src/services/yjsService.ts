...
const COLLAB_WS_BASE =
  import.meta.env.VITE_COLLAB_SERVICE_URL || "http://localhost:8083";
...
export class YjsService {
  ...
  connectWebSocket(
    documentId: string,
    user: any
  ): Promise<{ awareness: Awareness; provider: WebsocketProvider }> {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        this.provider = new WebsocketProvider(wsUrlWithParam, "", this.doc, {
          connect: true,
        });
  ...
  /**
   * Set initial content to Yjs document (only if document is empty)
  ...
  setInitialContent(content: string): void {
    try {
      ...
      // Only set content if document is empty
      if (xmlFragment.length === 0 && content && content.trim()) {
        console.log("📝 Setting initial content to Yjs document");
        // We'll let TipTap handle the content sync through its Collaboration extension
        // Just log that we're ready to receive content
      } else {
        console.log("ℹ️ Yjs document already has content or content is empty");
      }
    } catch (error) {
