...
// Connect to MongoDB
mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/smartnote')
...
export async function createCalendarReminder(userId, eventId, eventTitle, eventTime) {
  try {
    const notification = new Notification({
      ...
      relatedType: 'event',
      actions: [
        { label: 'View Event', url: `/calendar?event=${eventId}`, primary: true },
    ...
    await notification.save();
    console.log(`✅ [Notification] Calendar reminder created for event ${eventId}`);
    return notification;
  } catch (error) {
...
export async function createEventUpdateNotification(userId, eventId, eventTitle, changeType) {
  try {
    const messages = {
    ...
    const notification = new Notification({
      ...
      relatedType: 'event',
      actions: [
        { label: 'View Calendar', url: '/calendar', primary: true }
      ]
    });
...
export async function createWorkspaceInviteNotification(userId, workspaceName, inviterName) {
  try {
    const notification = new Notification({
      userId,
